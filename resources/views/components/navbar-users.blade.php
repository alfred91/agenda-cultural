<nav class="bg-red-600 shadow fixed min-w-full z-50" x-data="{ open: false }">
    <div class="max-w-7xl h-20 mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16">
            <div class="flex">
                <div class="flex-shrink-0 flex items-center mr-10">
                    <a href="{{ route('user.index') }}">
                        ...
                        <div class="shrink-0 flex items-center">
                            ...
                            <h1 class="block h-10 w-auto text-2xl mt-6 ml-2 text-white">
                                GARRUCHA
                ...
                <div class="hidden sm:block mt-4 sm:ml-6">
                    <div class="flex space-x-4">
                        <a href="{{ route('user.index') }}" class="px-3 py-2 rounded-md text-lg font-medium text-white uppercase hover:bg-red-700 transition duration-150 ease-in-out">Inicio</a>
                        <a href="{{ route('user.agenda') }}" class="px-3 py-2 rounded-md text-lg font-medium text-white uppercase hover:bg-red-700 transition duration-150 ease-in-out">Agenda</a>
                        <a href="{{ route('user.explore') }}" class="px-3 py-2 rounded-md text-lg font-medium text-white uppercase hover:bg-red-700 transition duration-150 ease-in-out">Explora</a>
                        <a href="{{ route('user.experiences') }}" class="px-3 py-2 rounded-md text-lg font-medium text-white uppercase hover:bg-red-700 transition duration-150 ease-in-out">Experiencias</a>
                    </div>
                </div>
            </div>
            ...
            <div x-data="{ isDropdownOpen: false }" class="relative hidden sm:flex sm:items-center sm:ml-6">
                ...
                <!-- Dropdown Menu -->
                <div x-show="isDropdownOpen" @click.away="isDropdownOpen = false" class="absolute z-50 w-48 rounded-md shadow-lg origin-top-right right-0 bg-white ring-1 ring-black ring-opacity-5 py-1" style="display: none; top: 3rem;">
    ...
    <div class="sm:hidden" x-show="open" @click.away="open = false">
        <div class="pt-2 pb-3 space-y-1">
            <a href="{{ route('user.index') }}" class="block px-3 py-2 rounded-md text-base font-medium text-white hover:bg-red-600 transition duration-150 ease-in-out">Inicio</a>
            <a href="{{ route('user.agenda') }}" class="block px-3 py-2 rounded-md text-base font-medium text-white hover:bg-red-600 transition duration-150 ease-in-out">Agenda</a>
